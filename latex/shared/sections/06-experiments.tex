...
tempering diagnostics produced strong committed policies but exposed a sharp
intermediate-temperature exchange bottleneck.  A 32-temperature, power-four,
lazy-kernel pilot improved the minimum adjacent-swap acceptance to 0.236,
reached a return effective sample size of 107 from 400 retained draws, and sent
eight distinct walkers to the cold replica.  It nevertheless completed no full
temperature round trip and left 11 policy coordinates without a retained-sample
switch.  These are tuning diagnostics, not evidence that the method outperforms
a competitor.  Full-scale arrays remain paused until longer warm and
overdispersed chains pass the predeclared between-chain gate.

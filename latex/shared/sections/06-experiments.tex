...
families use, respectively, \(8\!\times\!8\), \(16\!\times\!16\), and
\(32\!\times\!32\) grids; wall fractions of 9.4\%, 19.9\%, and 30.0\%; action
success probabilities of 0.90, 0.80, and 0.75; and horizons 64, 160, and 384.
Frozen-map audits record reachability, shortest-path length and count, and
single-cell start--goal bottlenecks.  Seed zero maps are development-only;
held-out maps are frozen before confirmation.  Small instances retain exact enumeration;
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{figures/gridworld-tiers.pdf}
  \caption{Representative development maps for the three GridWorld tiers.
  White cells are traversable, dark cells are walls, blue marks the start,
  and gold marks the goal.  Held-out maps are frozen separately and remain
  unused during method development and tuning.}
...
enumeration or known-model controls.  In the simulator-only stratum, fixed-tape
sample-average tempering and the five learned competitors receive the same
generative simulator and matched training-transition allowance.  Exact-model
...
sensitivity grid; the headline GridWorld target freezes one value on seed-zero
development maps.  The current development value is \(\beta=64\).  Any
additional value is reported as target sensitivity with separate mixing and
cost diagnostics, not as a best-of-grid result.
...
Confirmatory values are \textbf{TBD}.  Preliminary medium-GridWorld exact-value
...
The next predeclared checkpoint tests the exact whole-policy refresh in
Equation~\eqref{eq:global-policy-mh}, keeping the same ladder.  One
overdispersed 1,000-iteration run must accept a policy-changing post-burn
global proposal in the upper ladder quarter, accept a post-burn swap on the final edge, and send
at least two walkers to the cold replica while sampling at least two distinct
cold-replica policies.  Only then are one warm and one
overdispersed 2,500-iteration chain run; both must traverse the ladder and
every post-burn edge must accept at least \(0.20\).  These are tuning
diagnostics, not evidence that the method outperforms a competitor.  They do
not relax the later \(\widehat R\), ESS, round-trip, coordinate-agreement, and
within-chain movement gate.  Full-scale comparison arrays remain paused until
